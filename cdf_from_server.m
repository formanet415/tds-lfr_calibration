...
if strcmp(type, 'rswf')
    fPath = dir(fullfile('Z:\rpw\L2\tds_wf_e',sprintf('%4i/%02i/solo_L2_rpw-tds-surv-rswf-e-cdag_%4i%02i%02i*',year,month,year,month,day)));
    if size(fPath) == [1,1]
     	fname = fullfile(fPath.folder, fPath.name);
    elseif size(fPath) == [0,1]
        disp(['Missing data - file not found: ' fullfile(inputDIR,sprintf('%4i/%02i/solo_L2_rpw-tds-surv-rswf-e-cdag_%4i%02i%02i*',year,month,year,month,day))])
    elseif length(fPath) > 1
        disp('multiple files found, choosing highest version')
        names = struct2cell(fPath);
        names = names(1,:);
